...
- Put reusable logic in `src/matchcast/` and cover it with automated tests.
...
1. Implement the task and run its relevant validation or tests.
...
- [ ] Verify the Makefile install, test, and notebook commands on Windows.
...
- [ ] Store the untouched source file in `data/raw/` without manual edits.
...
- [ ] Define the processed match schema and data types.
